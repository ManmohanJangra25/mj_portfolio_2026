...
function domToReact(node, key = 'root') {
  ...
  for (const attribute of node.attributes) {
    ...
    } else if (name === 'value' && ['INPUT', 'TEXTAREA'].includes(node.tagName)) {
      props.defaultValue = attribute.value
    } else if (attribute.value === '' && ['download', 'disabled', 'checked', 'selected'].includes(name)) {
      props[name] = true
    } else {
...
function prepareSource() {
  const documentSnapshot = new DOMParser().parseFromString(sourceHtml, 'text/html')
...
function MobileMenu({ open, onNavigate }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center gap-8 md:hidden">
      <button
        type="button"
